...
REM set MSXEMUPATH=\AGD\V0.7.10\Suite MSX\fMSX
 set MSXEMUPATH=\AGD\Suite Colecovision\GearColeco
REM set EMU=fMSX.exe
 set EMU=GearColeco.exe
REM set MSXEMUPATH=f:\Emuladores\MSX\Emulators\blueMSX
...
REM OpenMSX DEFAULT MACHINES
REM ------------------------------
REM set MSX1MACHINE=Casio_PV-16
REM set MSX1MACHINE=Toshiba_HX-20E
REM set MSX1MACHINE=Casio_PV-7
set MSX1MACHINE=Sony_HB-75P
set MSX2MACHINE=Sony_HB-F500P
REM set MSX2MACHINE=Philips_NMS_8245
...
:createrom
	...
	copy "%AGDPATH%\*.asm" . >NUL 2>&1
        ...
        REM Convert filename to uppercase and save in file for header

	for /f "usebackq delims=" %%I in (`powershell "\"%NAME%\".toUpper()"`) do set "NAME=%%~I"
        echo       db "%NAME%/PRESENTS MPAGD/2025" >name.txt
